...
describe("ContactController", () => {
  ...
  describe("#addContact()", () => {
    it("should add a single contact into the book", done => {
      this.book
        .addContact("Alice", "001-101-1010", "alice@earth.com")
        .then(contact => {
          expect(contact.name).toBe("Alice");
          expect(contact.phone).toBe("001-101-1010");
          expect(contact.email).toBe("alice@earth.com");
          done();
        })
        .catch(err => {
          done();
  ...
  describe("#delete()", () => {
    ...
    it("should not remove any contact that do not match the ID passed", (done) => {
      this.book.addContact("Rick Deckard", "000-000-000", "null@null.com")
      .then((contact) => {
        this.book.getContacts()
        .then((contacts) => {
          expect(contacts[0].name).toBe(contact.name);
          expect(contacts.length).toBe(1);
          this.book.delete(99)
          .then(() => {
            this.book.getContacts()
    ...
    it("should remove the contact that matces the ID passed", (done) => {
      this.book.addContact("Rick Deckard", "000-000-000", "null@null.com")
      .then((contact) => {
        this.book.getContacts()
        .then((contacts) => {
